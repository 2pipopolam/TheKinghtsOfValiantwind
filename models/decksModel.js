...
function fromDBHospCardToCard(dbCard) {
    return new Card(dbCard.hosp_crd_id, dbCard.hosp_ugc_id,
        dbCard.hosp_crd_name,dbCard.hosp_ugc_active, dbCard.hosp_crd_img,dbCard.hosp_crd_type);
}

function fromDBHusCardToCard(dbCard) {
    return new Card(dbCard.hus_crd_id, dbCard.hus_ugc_id,
        dbCard.hus_crd_name,dbCard.hus_ugc_active, dbCard.hus_crd_img,dbCard.hus_crd_type);
}

function fromDBClassCardToCard(dbCard) {
    return new Card(dbCard.cla_crd_id, dbCard.ugcla_c_id,
        dbCard.cla_crd_name,dbCard.ugcla_c_active, dbCard.cla_crd_img,dbCard.cla_crd_type);
}



class Card {
    constructor(cardId, deckId, name,active, image,type) {
        this.cardId = cardId;
        this.deckId = deckId;
        this.name = name;
        this.active = active;
    ...
    static async genCard(playerId) {
        try {
            //playerStateId = 3 = Choice , playerStateId = 1 Ready , playerStateId = 2 = Acted , playerStateId = 4 = Mirror , playerStateId = 5 = Defense
            let [chosingState] = await pool.query(`select pl_state_id from player where pl_user_game_id = ?`, [playerId]);

            let playerStateId = chosingState[0].pl_state_id;
            ...
            if (playerStateId != 3 && chosenClass == 1 && ap >= 10) {
            ...
            if (playerStateId != 3 && chosenClass == 2 && ap >= 10) {
...
class MatchDecks {
    ...
    static async playDeckCard(game, deckId) {
        ...
        try {
            ...
            if (playerStateId != 3 && chosenClass == 1) {
            ...
            if (playerStateId != 3 && chosenClass == 2) {
                // get the card and check if the card is from the player and it is active
