...
class Play {
    ...
    static async startGame(game) {
        try {
            ...
            await pool.query(objSql, [p1Id, 3, Settings.maxHP, Settings.apPerTurn, 0]);
            await pool.query(objSql, [p2Id, 3, Settings.maxHP, 0, 0]);

            let [chosingState] = await pool.query(`select pl_state_id from player where pl_user_game_id = ?`, [game.player.id]);
            let stateId = chosingState[0].pl_state_id;
